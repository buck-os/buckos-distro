...
The data root, image cache, provision state, transactions, and configuration remain root-owned. Bind sources must be root-owned before container creation. After an existing container passes exact topology and runtime validation, an interrupted control-state or worker-scratch transition may be either root-owned or owned by the runtime's `nativelink` account. The worker state bind root remains root-owned; only its architecture-specific child becomes service-owned. Every accepted leaf is a real, non-writable directory with root-owned ancestry, and provisioning does not recursively change existing service data ownership.
...
Use `scripts/smoke-test.sh --help` for the bounded rollout. The sequence proves service readiness and cache-only reuse across clean clients. Each client must begin without `.buckconfig.local`; the harness exclusively installs the static RE client settings in a mode-0600 local file before Buck starts, audits those effective values without duplicate RE command-line overlays, and removes the exact installed file after daemon cleanup. Each architecture stage first forces an uncached remote `/usr/bin/uname -m` probe, parses structured `what-ran` evidence requiring the exact pinned `executor = Re` value, verifies the requested platform properties and selected execution platform, rejects local fallback or cache hits, and checks the returned canonical architecture. The `probe-x86_64` and `probe-aarch64` stages stop after that bounded attestation. The full architecture stages then run the small Debian `hostname` build as a separate real-workload check with distinct event evidence. This execution-backed attestation is required before broader clients or long builds are enabled, but it does not replace a future NativeLink worker-snapshot API. The smoke script does not contain an ISO target.
...
For an mTLS endpoint, pass `--tls true`, `--tls-ca`, `--tls-client-chain`, `--tls-client-key`, and the combined Buck2 identity through `--buck-tls-client-cert`. Add `--cross-host` for any published or routed endpoint; the harness rejects plaintext in that mode. The tracked mTLS NativeLink configs and client checks are ready for admission, but the SDME provisioner does not yet install their credentials or select them.
