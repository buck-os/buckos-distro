...
`tls_client_cert` is the combined Buck2 client certificate-chain and private-key PEM. The readiness helper takes the same identity as separate `--tls-client-chain` and `--tls-client-key` files. The smoke harness requires both client checkouts to start without `.buckconfig.local`, then atomically installs a mode-0600 file with the endpoint, instance, TLS mode, CA path, and combined identity path before the first Buck command. It removes only the exact installed files after killing its isolated daemons on every handled exit. TLS credentials are required in TLS mode, and `--cross-host` rejects plaintext. Provisioner support for installing and selecting the mTLS files is separate work.
